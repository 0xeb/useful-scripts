...
class preprocess_t(object):
    RE_BLOCK_BEGIN = re.compile('^.*!BLOCK_BEGIN ([^\s]+).*', re.I)
    RE_BLOCK_END   = re.compile('^.*!BLOCK_END ([^\s]+).*', re.I)
    RE_IFDEF       = re.compile('^.*!IFDEF (.*)', re.I)
    RE_ENDIF       = re.compile('^.*!ENDIF (.*)', re.I)
    RE_EXEC        = re.compile('^(.*)!EXEC (.*)', re.I)
    RE_VAR_WRAP    = re.compile('\$(\w+)\$', re.I)
